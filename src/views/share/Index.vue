<template>
  <div class="page">
    ...
    ></vertical-list>
    <order-status v-if="order && $store.state.ui.showStatus" :order="order"></order-status>
  </div>
</template>
  ...
  methods: {
    ...
    init: async function () {
      ...
      if (typeof id !== 'undefined') {
        ...
        try {
          if (isOld) {
            res_ = await axios.get(`${this.$store.state.config.urls.downUrl}${orderInfo.name}.json`)
          } else {
          ...
          console.error(error)
          res_ = await axios.get(`${'https://app.shareuc.com/'}${orderInfo.name}.json`)
        }
