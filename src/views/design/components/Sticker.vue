<template>
  <div :class="{ page: true, 'ma-2': $store.state.ui.os.isPC }">
    ...
    <input style="opacity: 0;" type="file" accept="*" ref="input" @change="onInputChange" />
  </div>
</template>
...
export default {
  ...
  watch: {
    ...
    '$store.state.ui.emojiIndex': function () {
      this.updateSwiper()
    }
  },
  computed: {
    list () {
      if (!this.$store.state.brand.brandJson) {
        ...
        return this.$store.state.brand.brandJson.emoji[this.$store.state.ui.emojiIndex].images
      }
    }
  },
  methods: {
    async onInputChange (data) {
      if (data.target.files && data.target.files.length > 0) {
        const file = data.target.files[0]
        data.target.value = ''
        try {
          const suffix = utils.getSuffix(file.name)
          const fname = 'UCAppGallery' + dayjs().format('YYYYMMDDHHmmss') + suffix
          const item = {
            file: file,
            url: '',
            src: '',
        ...
        for (let j = 0; j < count; j++) {
          ...
          if (!list[i * cols + j].id.startsWith('color~')) {
            var url = `url(${this_.$store.state.config.urls.galleryUrl}${list[
              i * cols + j
            ].id.replace(' ', '%20')}${this_.$store.getters.gallerySize})`
            slide += `<div style="flex: 0 0 ${100 /
              cols}%;background-position: ${pos};background-repeat: no-repeat;background-size: contain;background-image:
${url}" class="gallery~${list[i * cols + j].id}~${i * cols + j}"></div>`
          } else {
            var color = list[i * cols + j].id.replace('color~', '#')
            slide += `<div style="flex: 0 0 ${100 /
              cols}%;background-position: ${pos};background-repeat: no-repeat;background-size: contain;background-color:
...
contain;background-image: url(${url})" class="gallery~${
            ...
            slide += `<div style="flex: 0 0 ${100 /
              this_.row}%;height: ${itemWidth}px;background-position: bottom;background-repeat: no-repeat;background-size:
contain;background-color: ${color}" class="gallery~${
              list[i * this_.row + j].id
            }~${i * this_.row + j}"></div>`
          }
        }
        ...
        on: {
          ...
          click: e => {
            ...
            if (this.showEmoji) {
              ...
              if (texName === 'aurora/tex/upload.png') {
                this.$refs.input.click()
              } else if (texName === 'pantone') {
                ...
                if (texName.startsWith('color~')) {
                  ...
                  var src = `${this.$store.state.config.urls.galleryUrl}${texName}${this.$store.getters.textureSize}`
