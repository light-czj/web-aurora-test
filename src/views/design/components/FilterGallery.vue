<template>
  <div :class="{ page: true, 'ma-2': $store.state.ui.os.isPC }">
    <div v-if="$store.state.ui.os.isPC" ref="filters" style="width: 27vw;">
      <v-chip-group
        :show-arrows="true"
    ...
    <template v-if="$store.state.ui.os.isPC">
      <div
        ...
        :style="`height:${$store.state.ui.size.tabHeight - 16 - filtersHeight}px;`"
      >
    ...
    <v-dialog v-model="$store.state.ui.showFilters" max-width="500">
      <v-card style="max-height: 50vh;overflow-y: auto;" class="pa-2">
        <v-btn absolute icon style="top: 0;right: 0;z-index: 1;" @click="$store.state.ui.showFilters = false">
  ...
  computed: {
    list () {
      if (!this.$store.state.brand.brandJson) {
        ...
        return [{ id: 'pantone' }].concat(this.$store.state.brand.brandJson.texture[this.$store.state.ui.galleryIndex].images[0][this.tags[this.selectedTagIndex]])
      }
    },
    tags () {
      if (!this.$store.state.brand.brandJson) {
        ...
        return Object.keys(this.$store.state.brand.brandJson.texture[this.$store.state.ui.galleryIndex].images[0])
      }
    }
  },
  methods: {
    selectTag (index) {
      ...
      this.$store.state.ui.filter = this.tags[this.selectedTagIndex]
      this.updateSwiper()
    },
...
cover;background-color: ${color}" class="gallery~${
  ...
  mounted () {
    this_ = this
    if (this.$store.state.ui.os.isPC) {
      this.filtersHeight = parseFloat(getComputedStyle(this.$refs.filters).height)
    }
    this.parentHeight = parseFloat(getComputedStyle(this.$parent.$el).height) - this.filtersHeight
    if (this.$store.state.brand.loadJsonComplete) {
      this.initSwiper()
    }
  },
  activated () {
    this.$store.state.ui.showFilters = false
  }
}
</script>
